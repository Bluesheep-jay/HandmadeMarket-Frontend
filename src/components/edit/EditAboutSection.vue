<template>
  <div class="about-section">
    ...
    <div class="form-group">
      ...
      <div class="cate-container">
        <div v-if="!isEditingCategory">
          <div class="row">
            <div v-for="(category, index) in allCategoryParent" :key="index">
              <q-chip dense>{{ category.categoryName }}</q-chip>
            </div>
            <q-btn
              color="primary"
              label="Chỉnh sửa"
    ...
    <div class="form-group" v-if="hasPersonalization">
      <label for="personalizationDescription">Hướng dẫn cho người mua</label>
      <textarea
...
const existingCategory = ref(true);
const allCategoryParent = ref([]);
...
onBeforeMount(async () => {
  try {
    ...
    allCategoryParent.value = await categoryService.getAllParentCategories(
      props.productData.categoryId
    );

    allCategoryParent.value.reverse();

    // Kiểm tra nếu có personalizationDescription thì bật hasPersonalization
    if (props.productData.personalizationDescription) {
      hasPersonalization.value = true;
    }
  } catch (error) {
    console.log(error);
  }
...
function togglePersonalization() {
  ...
  if (!hasPersonalization.value) {
    props.productData.personalizationDescription = "";
  }
}
